...
#include "svcLayerMcu_messages.h"
#include "svcLayerMcu_server.h"
...
)
{
    ...
    _ServerDataPool = le_mem_CreatePool("svcLayerMcu_ServerData", sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    // Don't expect that to be more than 2-3, so use 3 as a reasonable guess.
    _HandlerRefMap = le_ref_CreateMap("svcLayerMcu_ServerHandlers", 3);
...
static void Handle_svcLayerMcu_SetMcuCommand1
(
...
)
{
    ...
    // Call the function
    le_result_t _result;
    _result  = svcLayerMcu_SetMcuCommand1 ( 
        value1 );
...
static void Handle_svcLayerMcu_SetMcuCommand2
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Call the function
    le_result_t _result;
    _result  = svcLayerMcu_SetMcuCommand2 ( 
        message );
...
static void Handle_svcLayerMcu_SetMcuCommand3
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Call the function
    le_result_t _result;
    _result  = svcLayerMcu_SetMcuCommand3 ( 
        value1, 
        message );
...
static void Handle_svcLayerMcu_GetMcuCommand1
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    {
        LE_DEBUG("Adjusting messageSize from %zu to 100", messageSize);
        messageSize = 100;
    }
    ...
    // Call the function
    le_result_t _result;
    _result  = svcLayerMcu_GetMcuCommand1 ( 
        message, 
        messageSize );
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_svcLayerMcu_SetMcuCommand1 : Handle_svcLayerMcu_SetMcuCommand1(msgRef); break;
        case _MSGID_svcLayerMcu_SetMcuCommand2 : Handle_svcLayerMcu_SetMcuCommand2(msgRef); break;
        case _MSGID_svcLayerMcu_SetMcuCommand3 : Handle_svcLayerMcu_SetMcuCommand3(msgRef); break;
        case _MSGID_svcLayerMcu_GetMcuCommand1 : Handle_svcLayerMcu_GetMcuCommand1(msgRef); break;
